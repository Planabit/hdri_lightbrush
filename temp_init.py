    ...
    def draw(self, context):
        ...
        if img.has_data and len(img.pixels) > 0:
            ...
            try:
                ...
                # Method A: Use grid layout for precise aspect ratio
                try:
                    scene.hdri_editor_current_image = img
                    
                    # Create a grid layout that maintains aspect ratio
                    grid_container = image_box.column(align=False)
                    
                    # Calculate grid dimensions based on aspect ratio
                    if aspect_ratio <= 0.5:  # Very wide image (panorama)
                        grid_rows = 1
                        scale_factor = 2.0 * user_scale
                    elif aspect_ratio <= 1.0:  # Wider than tall
                        grid_rows = 2
                        scale_factor = 3.0 * user_scale
                    elif aspect_ratio <= 2.0:  # Taller than wide
                        grid_rows = 4
                        scale_factor = 4.0 * user_scale
                    else:  # Very tall image
                        grid_rows = 6
                        scale_factor = 6.0 * user_scale
                    
                    # Create the main image display
                    for row_idx in range(grid_rows):
                        grid_row = grid_container.row(align=True)
                        
                        if row_idx == 0:  # First row contains the image
                            grid_row.scale_y = scale_factor
                            
                            # Use box inside row for better control
                            img_container = grid_row.box()
                            img_container.template_image(scene, "hdri_editor_current_image", compact=False)
                            
                        elif row_idx < grid_rows - 1:  # Middle rows - minimal spacers
                            grid_row.scale_y = 0.1
                            grid_row.separator()
                        else:  # Last row - info
                            grid_row.scale_y = 0.6
                            info_col = grid_row.column()
                            info_col.label(text=f"{img_width}×{img_height}")
                            info_col.label(text=f"Ratio: {aspect_ratio:.2f}:1")
        ...
        size_box = col.box()
        size_header = size_box.row()
        size_header.scale_y = 0.7
        size_header.label(text="Preview Size:", icon='FULLSCREEN_ENTER')
...
class HDRI_2D_SIMPLE_OT_fit_preview(Operator):
    """Fit preview to optimal size based on image aspect ratio"""
    bl_idname = "hdri_2d_simple.fit_preview"
    bl_label = "Auto Fit Preview"
    bl_description = "Automatically adjust preview scale to fit image aspect ratio"
    ...
    def execute(self, context):
        ...
        # Calculate optimal scale based on aspect ratio
        img_width, img_height = img.size[0], img.size[1]
        aspect_ratio = img_height / img_width if img_width > 0 else 1.0
        
        # Set optimal scale
        if aspect_ratio <= 0.3:  # Very wide panorama
            optimal_scale = 0.4
        elif aspect_ratio <= 0.6:  # Wide image
            optimal_scale = 0.7
        elif aspect_ratio <= 1.5:  # Standard ratio
            optimal_scale = 1.0
        elif aspect_ratio <= 2.5:  # Tall image
            optimal_scale = 1.3
        else:  # Very tall
            optimal_scale = 1.6
        
        # Apply the scale
        scene.hdri_preview_scale = optimal_scale
        ...
        self.report({'INFO'}, f"Auto-fitted preview (scale: {optimal_scale:.1f})")
        return {'FINISHED'}
